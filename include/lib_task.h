...
// t/speed секунд настенного времени от якоря. Если модель отстаёт от
// реального темпа, не спит и не догоняет скачком.
//
...
private:
    ...
    // --- исполнение ---
    UdpReader m_reader; // приёмный сокет задачи (порт-идентификатор)
    UdpSender m_sender; // сокет отправки адресатам
